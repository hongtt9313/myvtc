...
function findRecoveryAccounts(keyword) {
    ...
    const currentPage = window.location.pathname.split('/').pop() || 'MyVTC_Home.html';
            ...
            if (navLinks) {
                if (currentPage === 'Game.html' || currentPage === 'Shop.html' || currentPage === 'Support.html' || currentPage === 'RechargeDetail.html') {
            navLinks.innerHTML = '';
        } else {
            navLinks.innerHTML = `
...
function submitRecoveryPassword() {
    ...
    const targetTab = defaultTab || urlTab || 'personal';
...
function openCheckinMissionModal() {
    openLoyaltyModal(`
        <div class="loyalty-checkin-game">
            ...
            <div class="loyalty-checkin-grid game">
            ...
            <div class="loyalty-checkin-bonus game">
                <strong>Còn 4 ngày để nhận thưởng tuần.</strong>
                <span>Giữ chuỗi liên tục để mở phần thưởng +10 Point.</span>
            </div>

            <button type="button" class="loyalty-modal-primary game" onclick="checkinMission()">Điểm danh hôm nay</button>
        </div>
    `);
}
...
function setGameFilter(type) {
    activeGameFilter = type;

    document.querySelectorAll('[data-game-filter]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.gameFilter === type);
    });
...
function filterGameList() {
    ...
    list.querySelectorAll('.game-card').forEach(card => {
        ...
        const matchType = activeGameFilter === 'all' || activeGameFilter === type;
...
function filterShopGameList() {
    const list = document.getElementById('shop-game-list');
    ...
    list.querySelectorAll('.shop-game-card').forEach(card => {
        const type = card.dataset.type || '';
...
//  RECHARGE DETAIL
// ================================================================
const mockRechargeGames = [
    { slug: 'audition-pc', name: 'Audition PC', icon: 'AU', type: 'pc', linkedAccount: 'hongtran_AU', allowGuestTopup: true },
    { slug: 'dot-kich', name: 'Đột Kích', icon: 'CF', type: 'pc', linkedAccount: 'CF_HongTran', allowGuestTopup: false },
    { slug: 'truy-kich-pc', name: 'Truy Kích PC', icon: 'TK', type: 'pc', linkedAccount: '', allowGuestTopup: true },
    { slug: 'phi-doi', name: 'Phi Đội', icon: 'AO', type: 'pc', linkedAccount: '', allowGuestTopup: false },
    { slug: 'silkroad-origin-vtc', name: 'Silkroad Origin VTC', icon: 'SR', type: 'pc', linkedAccount: 'SilkHong90', allowGuestTopup: true },
    { slug: 'giang-ho-bat-phai', name: 'Giang Hồ Bát Phái', icon: 'GH', type: 'mobile', linkedAccount: '', allowGuestTopup: true },
    { slug: 'be-a-pro-football', name: 'Be A Pro Football', icon: 'BP', type: 'mobile', linkedAccount: 'BAP_HongTran', allowGuestTopup: true },
    { slug: 'football-pro-vtc', name: 'Football Pro VTC', icon: 'FP', type: 'mobile', linkedAccount: '', allowGuestTopup: true },
    { slug: 'dai-chien-tam-quoc', name: 'Đại Chiến Tam Quốc', icon: 'TQ', type: 'mobile', linkedAccount: '', allowGuestTopup: false },
    { slug: 'ngoi-nha-trong-mo', name: 'Ngôi Nhà Trong Mơ', icon: 'NM', type: 'mobile', linkedAccount: 'DreamHome88', allowGuestTopup: true }
];
...
function getRechargeGameBySlug(slug) {
    return mockRechargeGames.find(game => game.slug === slug) || mockRechargeGames[0];
}

function goRechargeDetail(gameName) {
    const slug = slugifyGameName(gameName || 'Audition PC');
    window.location.href = `RechargeDetail.html?game=${encodeURIComponent(slug)}`;
}

function initShopRechargeButtons() {
    document.querySelectorAll('.shop-recharge-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const holder = btn.closest('[data-name], [data-game]');
            const name = holder?.dataset.name || holder?.dataset.game || 'Audition PC';
            goRechargeDetail(name);
        });
    });
}

function getRechargePackages(game) {
    return [
        { id: 'pkg_1', name: `${game.name} - Gói 100 Point`, price: 100000, point: 100 },
        { id: 'pkg_2', name: `${game.name} - Gói 300 Point`, price: 300000, point: 300 },
        { id: 'pkg_3', name: `${game.name} - Gói 500 Point`, price: 500000, point: 500 },
        { id: 'pkg_4', name: `${game.name} - Gói 1.000 Point`, price: 1000000, point: 1000 },
        { id: 'pkg_5', name: `${game.name} - Gói ưu đãi`, price: 150000, point: 160 },
        { id: 'pkg_6', name: `${game.name} - Gói cao cấp`, price: 2000000, point: 2200 }
    ];
}
...
function initRechargeDetailPage() {
    ...
    const slug = new URLSearchParams(window.location.search).get('game') || 'audition-pc';
    const game = getRechargeGameBySlug(slug);
    rechargeState = {
        game,
        selectedPackage: getRechargePackages(game)[0],
        quantity: 1,
        payment: 'Số dư MyVTC',
        discount: 0
    };

    document.getElementById('recharge-game-icon').textContent = game.icon;
    document.getElementById('recharge-game-name').textContent = game.name;
...
function renderRechargeAccountBox() {
    ...
    if (!box || !rechargeState.game) return;

    const game = rechargeState.game;

    if (!isLoggedIn) {
        box.innerHTML = `
            <div class="recharge-login-box">
                <strong>Bạn cần đăng nhập để nạp game</strong>
                <span>Đăng nhập MyVTC để kiểm tra tài khoản chơi game và thanh toán.</span>
    ...
    if (game.linkedAccount) {
        box.innerHTML = `
            <div class="recharge-account-found">
                <strong>${game.linkedAccount}</strong>
                <span>Tên tài khoản đang chơi ${game.name}</span>
            </div>
        `;
        return;
    }

    if (game.allowGuestTopup) {
        box.innerHTML = `
            <div class="recharge-account-search">
                <strong>Nhập tên tài khoản chơi game</strong>
                <span>Game này cho phép nạp khi chưa liên kết tài khoản.</span>
    ...
    box.innerHTML = `
        <div class="recharge-account-missing">
            <strong>Bạn không có tài khoản của game này</strong>
            <span>Vui lòng đăng nhập tài khoản khác có chơi game này hoặc lựa chọn game khác.</span>
        </div>
    `;
}
...
function searchRechargePlayer() {
    ...
    if (!value) {
        showToast('Vui lòng nhập tên tài khoản chơi game', 'info');
        return;
    }
    rechargeState.game.linkedAccount = value;
    renderRechargeAccountBox();
    showToast('Đã tìm thấy tài khoản chơi game', 'success');
}

function renderRechargePackages() {
    const list = document.getElementById('recharge-package-list');
    if (!list || !rechargeState.game) return;

    list.innerHTML = getRechargePackages(rechargeState.game).map(pkg => `
        <button type="button" class="recharge-package-card ${pkg.id === rechargeState.selectedPackage.id ? 'active' : ''}" onclick="selectRechargePackage('${pkg.id}')">
            <div class="shop-game-icon">${rechargeState.game.icon}</div>
            <h3>${pkg.name}</h3>
            <p>${pkg.point.toLocaleString('vi-VN')} Point hoặc ${pkg.price.toLocaleString('vi-VN')}đ</p>
        </button>
    `).join('');
}

function selectRechargePackage(packageId) {
    rechargeState.selectedPackage = getRechargePackages(rechargeState.game).find(pkg => pkg.id === packageId) || rechargeState.selectedPackage;
...
function applyRechargeVoucher() {
    ...
    const subtotal = rechargeState.selectedPackage.price * rechargeState.quantity;

    if (code === 'MYVTC10') {
        rechargeState.discount = Math.min(50000, Math.round(subtotal * 0.1));
        if (message) message.textContent = `Áp dụng voucher thành công, giảm ${rechargeState.discount.toLocaleString('vi-VN')}đ`;
    } else {
        rechargeState.discount = 0;
        if (message) message.textContent = 'Mã hợp lệ để test: MYVTC10';
    }
...
function updateRechargeOrder() {
    ...
    const subtotal = pkg.price * rechargeState.quantity;
    ...
    document.getElementById('order-subtotal').textContent = `${subtotal.toLocaleString('vi-VN')}đ`;
    document.getElementById('order-discount').textContent = `${rechargeState.discount.toLocaleString('vi-VN')}đ`;
    document.getElementById('order-total').textContent = `${total.toLocaleString('vi-VN')}đ`;
}
...
const mockAccountNotifications = [
    ...
    { id: 'noti_3', icon: 'fa-gift', title: 'Voucher mới trong Túi đồ', body: 'Bạn nhận được voucher giảm 10% khi nạp game trong Shop.', time: '2 ngày trước', read: true },
